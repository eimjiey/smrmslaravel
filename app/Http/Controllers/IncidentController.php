...
{
    // 24-4634 or 24-4634-TS style IDs
    const STUDENT_ID_REGEX = '/^\d{2}-\d{3,4}(-TS)?$/';

    // Roles allowed to edit incidents
    const EDIT_ROLES = ['admin', 'staff', 'basic_filer', 'user'];

    // Disciplinary recommendation matrix
    const RECOMMENDATION_MATRIX = [
        ...
        'Major Offense' => [
            ...
            2 => 'Suspension from eleven (11) to fifteen (15) days.', // Duplicate key fixed
    ...
    private function authorizeDelete(User $user, Incident $incident)
    {
        ...
        // Filer authorization
        if ($incident->filer_id == $user->id) {   // note: loose compare is fine for "2" vs 2
            return null;
        }
    ...
    public function index(Request $request)
    {
        ...
        $mappedIncidents = $incidents->map(function ($incident) {
            ...
            return [
                ...
                'deleted_at'       => $incident->deleted_at,   // ← add this line
            ];
        });
    ...
    public function show($id)
    {
        ...
        // 1. Fetch Incident with all necessary relationships eager loaded
        ...
        // --- 3. Create Mapped Response Array ---
        
        // Safely extract program name/code
        $programName = $incident->student?->program?->code ?? $incident->student?->program?->name ?? 'N/A';
        
        $mappedIncident = [
            // Primary Incident Fields
            'id'                  => $incident->id,
            'incidentId'          => $incident->id, // For Flutter model consistency
            'student_id'          => $incident->student_id, // Student number string
            ...
            'actionTaken'         => $incident->disciplinary_action, // Mapped from DB column name
            
            // Student/Program Display Fields (Extracted from relationships)
    ...
    public function store(Request $request)
    {
        ...
        try {
            $validatedData = $request->validate([
                ...
                'dateOfIncident' => ['required', 'date', function ($attribute, $value, $fail) { if (date('w', strtotime($value)) == 0) { $fail('The date of incident cannot be a Sunday.'); } }],
                'timeOfIncident' => ['required', 'date_format:H:i', function ($attribute, $value, $fail) { $start = strtotime('07:00'); $end = strtotime('17:00'); $inputTime = strtotime($value); if ($inputTime < $start || $inputTime > $end) { $fail('The time of incident must be between 7:00 AM and 5:00 PM.'); } }],
                'location' => 'required|string|max:255',
                'description' => 'required|string',
                'actionTaken' => 'nullable|string|max:255',
                'fullName' => 'nullable|string', 'program' => 'nullable|string', 'yearLevel' => 'nullable|string', 'section' => 'nullable|string',
            ]);

            $studentIdInput = $validatedData['student_id'];
            $student = Student::where('student_number', $studentIdInput)->first();
            if (!$student) { return response()->json(['message' => 'The given data was invalid.', 'errors' => ['studentNumber' => ["The student with ID {$studentIdInput} does not exist in the record."]]], 422); }

            $offenseCategory = OffenseCategory::where('name', $validatedData['offenseCategory'])->first();
            if (!$offenseCategory) { return response()->json(['message' => 'The given data was invalid.', 'errors' => ['offenseCategory' => ['Invalid offense category provided.']]], 422); }

            $specificOffense = Offense::where('category_id', $offenseCategory->id)->where('name', $validatedData['specificOffense'])->first();
            if (!$specificOffense) { return response()->json(['message' => 'The given data was invalid.', 'errors' => ['specificOffense' => ['Invalid specific offense provided for the selected category.']]], 422); }

            $recommendation = $this->getDisciplinaryRecommendation($studentIdInput, $offenseCategory->name);
            ...
            return response()->json(['message' => 'Incident report filed successfully.', 'incident' => $incident, 'recommendation' => $recommendation], 201);
        } catch (ValidationException $e) {
            $errorMap = [];
            foreach ($e->errors() as $key => $messages) { $errorMap[Str::camel($key)] = $messages; }
            return response()->json(['message' => 'Validation failed. Please correct the highlighted fields.', 'errors' => $errorMap], 422);
        } catch (\Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($incident->trashed() && $request->input('status') !== 'Deleted') { return response()->json(['message' => 'Cannot update a deleted incident.'], 400); }

        try {
            $canEdit = ($user->role === 'admin' || $incident->filer_id === $user->id || in_array($user->role, self::EDIT_ROLES));

            if (!$canEdit) { return response()->json(['message' => 'Unauthorized action. You must be the filer or have a designated administrative role to edit this report.'], 403); }
            ...
            if (isset($validatedData['studentId'])) { $updateData['student_id'] = $validatedData['studentId']; }
            if (isset($validatedData['categoryId'])) { $updateData['category_id'] = $validatedData['categoryId']; }
            if (isset($validatedData['specificOffenseId'])) { $updateData['specific_offense_id'] = $validatedData['specificOffenseId']; }
            if (isset($validatedData['actionTaken'])) { $updateData['disciplinary_action'] = $validatedData['actionTaken']; }

            if (isset($updateData['status']) && $updateData['status'] === 'Deleted') {
                if ($user->role === 'admin' || $incident->filer_id == $user->id) { $incident->delete(); return response()->json(['message' => 'Incident report soft-deleted successfully.'], 200); }
                return response()->json(['message' => 'Unauthorized: Only Admin or the original Filer can soft-delete this report.'], 403);
            }
            ...
            if ($user->role === 'admin' && ($request->has('categoryId') || $request->has('studentId'))) {
                ...
                if ($offenseCategory) {
                    $calculatedRecommendation = $this->getDisciplinaryRecommendation($newStudentId, $offenseCategory->name, $incident->id);

                    if (!isset($updateData['disciplinary_action'])) { $updateData['disciplinary_action'] = $calculatedRecommendation; }
                }
            }
            ...
            $errorMap = [];
            foreach ($e->errors() as $key => $messages) { $errorMap[Str::camel($key)] = $messages; }
            return response()->json(['message' => 'Validation Failed: One or more fields are invalid.', 'errors' => $errorMap], 422);
        } catch (\Exception $e) {
    ...
    // ==============================================================================
    // Status / Action Taken / Delete / Restore / Force Delete
    // ==============================================================================
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        $incident = Incident::withTrashed()->find($id);
        if (!$incident) { return response()->json(['message' => 'Incident not found.'], 404); }
        if ($incident->trashed()) { return response()->json(['message' => 'Cannot update status of a deleted incident.'], 400); }
        if ($user->role !== 'admin') { return response()->json(['message' => 'Unauthorized to update incident status.'], 403); }

        $validated = $request->validate(['status' => 'required|string|in:Pending,Investigation,Resolved,Closed']);
    ...
    public function updateActionTaken(Request $request, $id)
    {
        ...
        $incident = Incident::withTrashed()->find($id);
        if (!$incident) { return response()->json(['message' => 'Incident not found.'], 404); }
        if ($incident->trashed()) { return response()->json(['message' => 'Cannot update action taken for a deleted incident.'], 400); }
        if ($user->role !== 'admin') { return response()->json(['message' => 'Unauthorized to record action taken.'], 403); }

        try {
            $validated = $request->validate(['actionTaken' => 'required|string|max:255']);
        ...
        } catch (ValidationException $e) { return response()->json(['message' => 'The given data was invalid.', 'errors' => $e->errors()], 422);
        } catch (\Exception $e) {
    ...
    public function destroy($id)
    {
        ...
        if ($incident->trashed()) { return response()->json(['message' => 'Incident is already deleted.'], 400); }
    ...
    public function restore($id)
    {
        $user = Auth::user();
        if (!$user) { return response()->json(['message' => 'Unauthenticated.'], 401); }
        if ($user->role !== 'admin') { return response()->json(['message' => 'Unauthorized to restore incidents.'], 403); }
    ...
    public function forceDelete($id)
    {
        $user = Auth::user();
        if (!$user) { return response()->json(['message' => 'Unauthenticated.'], 401); }
        if ($user->role !== 'admin') { return response()->json(['message' => 'Unauthorized to permanently delete incidents.'], 403); }
    ...
    private function getDisciplinaryRecommendation(string $studentId, string $offenseCategoryName, $excludeIncidentId = null): string
    {
        ...
        if ($excludeIncidentId !== null) { $query->where('id', '!=', $excludeIncidentId); }
        ...
        if (empty($matrix)) { return 'No specific recommendation found for this category.'; }
        if (isset($matrix[$offenseNumber])) { return $matrix[$offenseNumber]; }

        $keys = array_keys($matrix);
        $maxKey = end($keys);

        if ($offenseNumber > $maxKey) { return $matrix[$maxKey]; }
